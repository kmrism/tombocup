...
- 書き込み先：Googleスプレッドシート「2026 参加者管理 得点申告表 受付表」（ドライブ「とんぼクラブ」フォルダ）の `参加者一覧`。
  ...
  列：E No. / F 申込日時（「9月22日」改行「14:44」の文字列） / G・H クラブ員・ビジター◯ / I 名前 / J フリガナ … AB LT24（郵便番号は保存しない）。`受付用` などは `参加者一覧` を参照している
- `ログ集計用` の C 列（名前の右）には、参加者一覧のフリガナから作ったローマ字「名 姓」（ヘボン式・長音省略）を GAS が書き込む。
  申込のたびに全員分を作り直す（GAS の `updateRomaji` を手動実行しても更新できる）。外国籍の人など不自然な綴りは手で直す
